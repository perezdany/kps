        ...
        <div class="col-md-6">
          ...
          <div class="box box-info">
            ...
            @foreach($get as $reservation)
                <form class="form-horizontal" action="attempt_to_pay" method="post">
                    @csrf
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                          <label  class="col-sm-4 control-label">Date de réservation(*) :</label>

                          <div class="col-sm-8">
                            <input type="date" class="form-control"  name="date_debut" value="{{$reservation->date}}"required>
                          </div>
                        </div>
                        ...
                        <div class="form-group"><!--LES DONNEES A INSERER POUR LE PAIMENT PAR TRANCHE-->
                            
                            <label  class="col-sm-6 control-label">Paiement choisi : {{$reservation->libele_paiement}}</label>
                            <div class="col-sm-6"> 
                                @if($reservation->id_paiement == 3)
                                    @php
                                        $to_pay = $reservation->montant / 2;
                                        $rest =  $reservation->montant - $reservation->montant_paye;
                                    @endphp 
                                    <b>RESTE A PAYER :</b> {{$rest}}<br>
                                    <b>MONTANT A PAYER : </b>{{$to_pay}}
                                    <input type="text" class="form-control" name="pay" value="{{$to_pay}}" style="display:none;">
                                @endif
                                @if($reservation->id_paiement == 4)
                                    @php
                                        $to_pay = $reservation->montant / 3;
                                        $rest =  $reservation->montant - $reservation->montant_paye;
                                    @endphp
                                    <input type="text" class="form-control" name="pay" value="{{$to_pay}}" style="display:none;">
                                    <b>RESTE A PAYER :</b> {{$rest}} <br>
